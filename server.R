...
shinyServer(function(input, output) {
  ...
  #---------------------------------------------------------------
  # Load processed data from amazon and subset by selected species
  #---------------------------------------------------------------
  
  # Load data from S3
  
  
  dat_all <- aws.s3::s3read_using(FUN = read_csv,
  ...
  # arrange the factor levels for fish traveling upstream
    node_vec <- dat_all %>%
  ...
  tmp_dat <- reactive({
    dat_all[dat_all$Mark.Species == input$pit_spp,]  # will need to add stream name to dataset if we were to add more watersheds, also year?
  })
  ...
  #------------------------------------------
   ...
   output$mig_plot <- renderPlot({
     ...
     lims <- c(ymd_hms("20180501 00:00:00"), Sys.time())
     tmp_tag <- selected_tag_ids()
     
       tmp_dat() %>%
       #tmp_dat %>%
       filter(TagID %in% tmp_tag) %>%
       ggplot(aes(x = firstObsDateTime, y = Node, group = TagID, colour = Release.Site.Code), drop = ) +
      ...
      geom_vline(xintercept = as.numeric(ymd_hms("20180611 15:00:00")), linetype=2)+
       ...
       #scale_y_continuous(breaks = axis_label$RiverKM, labels = axis_label$Node) +
